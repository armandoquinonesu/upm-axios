 ...
 * @param {AxiosProxyConfig} config configuration from Axios options object
...
function setProxy(options, config, location) {
  ...
  if (proxy) {
    ...
    if (isHttps.test(options.protocol)) {
      ...
      var proxyAgentOptions = utils.merge(tlsOptions, agentOptions, proxy);
      ...
      options.agents.https = new httpsProxyAgent(proxyAgentOptions);
